...
namespace mt_kahypar::dyn {
  ...
  inline void print_progress_bar(size_t i, size_t total, size_t km1, double moved_nodes, double cut) {
      // clear the line
      std::cout << "\r\033[K" << std::flush;
      std::string output = "";
      output += "km1: " + std::to_string(km1) + ", cut:" + std::to_string(cut) + ", moved nodes:" + std::to_string(moved_nodes);
      output += "    [";
  ...
  inline std::vector<PinChange> parsePins_fast(const std::string& line) {
    ...
    inline void log_km1_live(size_t i, size_t max_changes, Context& context, ds::PartitionedHypergraph<ds::MutableHypergraph>& hypergraph, std::chrono::duration<double> time) {
      ...
      file << i << ", " << km1 << ", " << imbalance << ", " << std::chrono::duration_cast<std::chrono::milliseconds>(time).count() << ", "
           ...
           << std::chrono::duration_cast<std::chrono::milliseconds>(context.dynamic.add_node_duration_sum).count() << ", "
           << std::chrono::duration_cast<std::chrono::milliseconds>(context.dynamic.remove_node_duration_sum).count() << ", "
           << std::chrono::duration_cast<std::chrono::milliseconds>(context.dynamic.add_edge_duration_sum).count() << ", "
           << std::chrono::duration_cast<std::chrono::milliseconds>(context.dynamic.remove_edge_duration_sum).count() << ", "
           << std::chrono::duration_cast<std::chrono::milliseconds>(context.dynamic.add_pin_duration_sum).count() << ", "
           << std::chrono::duration_cast<std::chrono::milliseconds>(context.dynamic.remove_pin_duration_sum).count() << ", "
      ...
      if (!context.dynamic.server) {
        ...
        print_progress_bar(i, max_changes, km1, context.dynamic.move_count, cut);
      }
