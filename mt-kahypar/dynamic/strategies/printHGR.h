...
namespace mt_kahypar::dyn {
    ...
    class Print : public DynamicStrategy {
    ...
    private:
        ...
        void print_partitioned_hypergraph() {
          if (!partitioned_hypergraph_s) {
            std::cout << "Partitioned hypergraph is not initialized" << std::endl;
            return;
          }
          //print number of active vertices:
          size_t num_active_vertices = 0;
          for (size_t i = 0; i < partitioned_hypergraph_s->initialNumNodes(); ++i) {
            if (partitioned_hypergraph_s->nodeIsEnabled(i)) {
              num_active_vertices++;
            }
          }
          std::cout << num_active_vertices << std::endl;
          //print ids of active vertices:
          for (size_t i = 0; i < partitioned_hypergraph_s->initialNumNodes(); ++i) {
            if (partitioned_hypergraph_s->nodeIsEnabled(i)) {
              std::cout << i << std::endl;
            }
          }
          //print number of active edges:
          size_t num_active_edges = 0;
          for (size_t i = 0; i < partitioned_hypergraph_s->initialNumEdges(); ++i) {
            if (partitioned_hypergraph_s->edgeIsEnabled(i)) {
              num_active_edges++;
            }
          }
          std::cout << num_active_edges << std::endl;
          //print ids of active edges and their pins
          for (size_t i = 0; i < partitioned_hypergraph_s->initialNumEdges(); ++i) {
            if (partitioned_hypergraph_s->edgeIsEnabled(i)) {
              std::cout << i;
              for (const HypernodeID& hn : partitioned_hypergraph_s->pins(i)) {
                if (partitioned_hypergraph_s->nodeIsEnabled(hn)) {
                  std::cout << " " << hn;
                }
              }
              std::cout << std::endl;
          ...
          std::cout << "Partitioned hypergraph:" << std::endl;
          for (size_t i = 0; i < partitioned_hypergraph_s->initialNumNodes(); ++i) {
            if (partitioned_hypergraph_s->nodeIsEnabled(i)) {
              std::cout << i << " " << partitioned_hypergraph_s->partID(i) << std::endl;
            }
          }
        }
