            ...
            ("p-obj-func",
             po::value<std::string>()->value_name("<string>")->notifier(
                     [&](const std::string& type) {
                       context.preprocessing.obj_func = clusteringObjFuncFromString(type);
                     })->default_value("multiply"),
             "Objective function for separated nodes clustering:\n"
             "- multiply\n"
             "- squared")
  ...
  po::options_description createCoarseningOptionsDescription(Context& context,
    ...
    options.add_options()
            ...
            ("c-outward-weight-ratio",
             po::value<double>(&context.coarsening.outward_weight_ratio)->value_name("<double>")->default_value(1),
             "Ratio of acceptable outward weight when using c-forbid-different-density-contractions (in [0, 1]).")
