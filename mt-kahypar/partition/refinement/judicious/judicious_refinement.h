...
public:
  explicit JudiciousRefiner(Hypergraph &hypergraph, const Context &context)
      : _hypergraph(hypergraph), _context(context),
...
private:
  bool
  refineImpl(PartitionedHypergraph &hypergraph,
             const parallel::scalable_vector<HypernodeID> &refinement_nodes,
             kahypar::Metrics &best_metrics, double) final;

  void initializeImpl(PartitionedHypergraph &phg) final;
  void finalizeRefinementRound(const PartitionedHypergraph &phg,
...
private:
  ...
  ds::ExclusiveHandleHeap<ds::MaxHeap<HyperedgeWeight, PartitionID>>
      _part_loads;
