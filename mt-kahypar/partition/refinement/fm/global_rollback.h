...
struct BestIndexReduceBody {
  ...
  BestIndexReduceBody(const vec<Gain>& gains, const boost::dynamic_bitset<>& in_balance) :
          gains(gains),
          in_balance(in_balance) { }

  BestIndexReduceBody(BestIndexReduceBody& b, tbb::split) :
          gains(b.gains),
          in_balance(b.in_balance) { }
