...
class RecursiveInitialPartitionerT : public IInitialPartitioner, tbb::task {
 private:
  using HyperGraph = typename TypeTraits::HyperGraph;
  using StreamingHyperGraph = typename TypeTraits::StreamingHyperGraph;
  using TBB = typename TypeTraits::TBB;
  using HwTopology = typename TypeTraits::HwTopology;
  using Self = RecursiveInitialPartitionerT<TypeTraits>;
  ...
  /*RecursivePartitionResult recursivePartition(const size_t num_threads, const TaskGroupID task_group_id, const size_t recursion_number) {
    ...
    auto copy = _hg.copy(result.context.partition.k, task_group_id);
    ...
    multilevel::partition(result.hypergraph, result.context, false, task_group_id);
  ...
  class RecursiveChildTask : public tbb::task {
   public:
    ...
    tbb::task* execute() override {
      ...
      utils::Timer::instance().start_timer("top_level_multilevel_recursion_" + std::to_string(_recursion_number),
                                          "Top Level Multilevel Recursion " + std::to_string(_recursion_number), true, _top_level);
      multilevel::partition(_result.hypergraph, _result.context, false, _task_group_id);
      utils::Timer::instance().stop_timer("top_level_multilevel_recursion_" + std::to_string(_recursion_number), _top_level);
 ...
 public:
  ...
  tbb::task* execute() override {
    ...
    if (do_parallel_recursion) {
      ...
      RecursiveChildTask& recursion_0 = *new(tbb::task::allocate_child()) RecursiveChildTask(
        _hg, _context, r1, _top_level, tbb_recursion_task_groups.first, num_threads_1, 0);
      RecursiveChildTask& recursion_1 = *new(tbb::task::allocate_child()) RecursiveChildTask(
        _hg, _context, r2, _top_level, tbb_recursion_task_groups.second, num_threads_2, 0);
      tbb::task::set_ref_count(3);
      tbb::task::spawn(recursion_1);
      tbb::task::spawn_and_wait_for_all(recursion_0);
      ...
      RecursiveChildTask& recursion = *new(tbb::task::allocate_child()) RecursiveChildTask(
        _hg, _context, best, _top_level, _task_group_id, _context.shared_memory.num_threads, 0);
      tbb::task::set_ref_count(2);
      tbb::task::spawn_and_wait_for_all(recursion);
    }
 ...
 private:
  void initialPartitionImpl() override final {
    if (_top_level) {
      ...
      Self& root_recursive_task = *new(tbb::task::allocate_root()) Self(
      ...
      LOG << "CHILD";
      Self& child_recursive_task = *new(tbb::task::allocate_child()) Self(
        _hg, _context, _top_level, _task_group_id);
      tbb::task::set_ref_count(2);
      tbb::task::spawn_and_wait_for_all(child_recursive_task);
    }
  }
