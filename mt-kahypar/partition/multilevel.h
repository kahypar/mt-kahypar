...
class RefinementTask : public tbb::task {
  ...
  using HeavyHyperedgeRemover = HeavyHyperedgeRemoverT<GlobalTypeTraits>;
 ...
 public:
  RefinementTask(Hypergraph& hypergraph,
    ...
    _coarsener(nullptr),
    _heavy_he_remover(context, task_group_id),
    _hg(hypergraph),
...
class CoarseningTask : public tbb::task {
 ...
 public:
  CoarseningTask(Hypergraph& hypergraph,
                 HeavyHyperedgeRemover& heavy_he_remover,
                 const Context& context,
                 ICoarsener& coarsener,
                 const bool top_level,
                 const TaskGroupID task_group_id) :
    _hg(hypergraph),
    _heavy_he_remover(heavy_he_remover),
    _context(context),
  ...
  tbb::task* execute() override {
    ...
    if ( _context.initial_partitioning.use_heavy_hyperedge_removal ) {
      ...
      _heavy_he_remover.sparsify(_coarsener.coarsestHypergraph());
      utils::Timer::instance().stop_timer("sparsify_hypergraph");

      if (_context.partition.verbose_output) {
        mt_kahypar::io::printHypergraphInfo(
          _heavy_he_remover.sparsifiedHypergraph(), "Sparsified Hypergraph",
          _context.partition.show_memory_consumption);
      }
      initialPartition(_heavy_he_remover.sparsifiedPartitionedHypergraph());
    } else {
...
static void spawn_multilevel_partitioner(Hypergraph& hypergraph,
  ...
  CoarseningTask& coarsening_task = *new(refinement_task.allocate_child()) CoarseningTask(
    hypergraph, refinement_task._heavy_he_remover,
