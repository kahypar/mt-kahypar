...
###################################
algorithm = "PT-Scotch"
scotch = os.environ.get("PTSCOTCH")
...
scotch_proc = subprocess.Popen(["mpirun -N " +str(args.threads) + " " +
                                scotch + " " +
                                str(args.k) + " " +
                                str(args.graph) + " " +
                                "-b" +str(args.epsilon) + " " +
                                "-cq" + " " +
                                "-vmts"], stdout=subprocess.PIPE, universal_newlines=True, shell=True)
...
if scotch_proc.returncode == 0:
  ...
  for line in out.split('\n'):
    ...
    if ("CommCutSz" in s):
      ...
      km1 = cut
    if ("Mapping" in s):
      total_time = float(s.split()[3].split("=")[1])
    if ("Target" in s):
