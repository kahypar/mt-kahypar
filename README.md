...
- `deterministic`: configuration for fast deterministic partitioning
- `deterministic_quality`: configuration for high-quality deterministic partitioning (uses flow-based refinement)
- `large_k`: configuration for partitioning (hyper)graphs into a large number of blocks (e.g. >= 1024 blocks)

The presets can be ranked from lowest quality to highest quality as follows: `large_k`, `deterministic`,
`default`, `deterministic_quality`, `quality`, and `highest_quality` (the quality of the equivalent deterministic and non-deterministic configurations is roughly equivalent).
